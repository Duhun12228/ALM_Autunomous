...
>
> **이 브랜치(`main`) = 방식 A**: FAST-LIO2 매핑 + ICP 재측위. 초기위치는 **사람이
> 지정**합니다(RViz "2D Pose Estimate" 또는 `initial_*` 파라미터) → icp_node 가
> prior map 에 정밀 정합 → FAST-LIO 가 추적. 가장 단순하고 안정적인 레퍼런스
> 구현으로, 자동 초기화(방식 B/C)의 비교 기준선입니다.
...
**← 이 브랜치 = 방식 A (`main`)**. 세 방식 공통: 추적(pose tracking)은 FAST-LIO,
경로계획은 Nav2, 구동은 STM32. 방식 A→B→C 순으로 무겁고 정교해집니다.
