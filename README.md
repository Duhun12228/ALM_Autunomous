...
>
> **이 브랜치(`dev/sc-lio-sam`) = 방식 C**: 매핑을 FAST-LIO2 대신 **SC-LIO-SAM**
> (LIO-SAM ros2 + Scan Context 루프클로저, GTSAM 팩터그래프)으로 교체해 넓은 공간의
> 드리프트를 보정합니다 (`slam_sc.launch.py`). 방식 B(SC 초기위치 자동특정)도
> 머지되어 있어, 만든 맵으로 `sc_build_db.py` → `localization.launch.py`(auto_init)
> 재측위까지 동일하게 사용합니다. GTSAM 은 소스빌드가 필요합니다.
...
| `dev/fastlio2-sc` | B | FAST-LIO2 | Scan Context 자동 (`sc_localizer`) | GTSAM 불필요, 매핑 무변경 |
| **`dev/sc-lio-sam`** | **C** | **SC-LIO-SAM**(루프클로저) | **Scan Context 자동** | **넓은 공간 드리프트 보정, GTSAM 빌드 필요** |

**← 이 브랜치 = 방식 C (`dev/sc-lio-sam`)**. 세 방식 공통: 추적(pose tracking)은
...
# 2-C) [방식 C, 이 브랜치] 매핑을 SC-LIO-SAM 으로 (루프클로저 = RS + Scan Context)
ros2 launch alm_sensors lidar.launch.py                 # 터미널1: 센서
ros2 launch alm_navigation slam_sc.launch.py rviz:=true # 터미널2: SC-LIO-SAM 4노드+IMU필터
#    루프를 닫으면 콘솔에 "SC loop found!"/"Add this SC loop" 로그 → 그래프 보정
#    맵 저장(종료 전):
ros2 service call /lio_sam/save_map lio_sam/srv/SaveMap "{resolution: 0.0, destination: ''}"
#    → $MAPS/sc_lio_sam/GlobalMap.pcd  (이후 pcd2pgm/sc_build_db/측위는 동일 절차,
#      pcd 경로만 GlobalMap.pcd 로 지정)

# 공개 MID-360 ROS 2 bag 테스트 (AIST Driving SLAM Test, 약 517 MB)
# https://zenodo.org/records/14841855 에서 rosbag2_2024_04_16-14_17_01.zip 다운로드/압축해제 후:
ros2 launch alm_navigation slam_sc.launch.py rviz:=true imu_accel_scale:=9.80665 n_scan:=4
# 별도 터미널: driver2 timestamp(ns)→time(s), line→ring 변환
ros2 run alm_sensors add_time_field.py --ros-args \
  -p input_topic:=/bag/lidar -p output_topic:=/livox/lidar
# 별도 터미널: LiDAR만 adapter 입력으로 remap, IMU는 원래 토픽 유지
ros2 bag play /path/to/rosbag2_2024_04_16-14_17_01 \
  --topics /livox/lidar /livox/imu --remap /livox/lidar:=/bag/lidar
# 장거리/경사 맵의 2D 변환: 전역 z 대신 trajectory 기준 상대 높이 사용
ros2 run alm_navigation pcd2pgm.py \
  --pcd $MAPS/sc_lio_sam_mid360_test/GlobalMap.pcd \
  --trajectory $MAPS/sc_lio_sam_mid360_test/trajectory.pcd \
  --out $MAPS/sc_lio_sam_mid360_test/mid360_2d_map \
...
# 2.5) [방식 B] Scan Context DB 생성 (맵 갱신 때마다 1회, 오프라인)
ros2 run alm_navigation sc_build_db.py --pcd $MAPS/alm_3d_map.pcd \
  --out $MAPS/sc_db.npz --selftest 20    # 가상스캔 자가검증 포함 (성공률 확인)
#    z밴드 기본 [-0.3, 1.0]: 반드시 천장 아래로 (천장 포함 시 장소구분 무너짐)

# 3) 측위만 검증 (FAST-LIO-Localization + SC 자동초기화)
ros2 launch alm_sensors lidar.launch.py                # 센서
ros2 launch alm_navigation localization.launch.py      # sc_localizer + icp + transform_publisher + fastlio
