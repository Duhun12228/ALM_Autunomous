...
> Estimate)하는 대신 **Scan Context 로 자동특정**합니다. `sc_build_db.py` 로 맵에서
> SC DB 를 1회 생성해 두면, 부팅 시 `sc_localizer` 가 현재 스캔을 DB 와 대조해
> `/initialpose` 를 자동 발행 → 기존 icp_node/FAST-LIO 후단은 방식 A 그대로.
