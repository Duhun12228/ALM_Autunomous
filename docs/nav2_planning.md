...
진짜 제자리 회전이 필요하면 Nav2 는 `vx ≈ 0, wz` 큰 명령을 냅니다. 그건 기존
임계값(`auto_spin_angular_threshold` 0.35 AND `auto_spin_linear_threshold` 0.04)이
잡습니다.

> ⚠ **사각지대**: `vx < 0.03` 이면서 `|wz| < 0.35` 인 구간은 wz 가 0 으로 접히고
> spin 도 안 걸립니다. MPPI 는 진척이 없으면 wz 를 키우므로 스스로 임계값을
> 넘어 빠져나옵니다. 실차에서 이 구간에 머무는 게 보이면
> `auto_spin_angular_threshold` 를 낮추세요 (0.35 → 0.20).
