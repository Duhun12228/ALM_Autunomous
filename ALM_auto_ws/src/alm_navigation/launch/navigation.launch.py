...
구성:
  ...
  nav2 navigation_launch: planner(SmacPlanner2D) / smoother(ConstrainedSmoother)
                          / controller(MPPI) / behavior / bt -> /cmd_vel

경로 파이프라인:
  ComputePathToPose(SmacPlanner2D) -> SmoothPath(ConstrainedSmoother) -> FollowPath(MPPI)
...
def generate_launch_description():
    ...
    # (SmoothPath 노드가 들어간 커스텀 트리 = ConstrainedSmoother 가 실제로 호출되는 지점)
