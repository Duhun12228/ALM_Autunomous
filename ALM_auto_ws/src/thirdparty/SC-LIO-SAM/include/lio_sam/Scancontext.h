...
// ALM: opencv/cv_bridge include 제거 — Scancontext 는 cv 를 전혀 쓰지 않음
// (원본 SC-LIO-SAM 의 잔재. Jetson 에 core 외 OpenCV dev 헤더가 없어 제거 필요)
...
public:
    // hyper parameters ()
    // ALM: MID-360 실내(스마트팜/팩토리) 튜닝 — 원본은 KITTI 실외(높이 2.0 m, 반경 80 m)
