...
// ALM: Livox MID-360 UDP 파서(alm_sensors/livox_udp_pointcloud2) 출력 포맷.
// x,y,z,intensity + optional native ring + per-point relative time.
struct LivoxPointXYZIT
{
    PCL_ADD_POINT4D
    PCL_ADD_INTENSITY;
    uint16_t ring;
...
POINT_CLOUD_REGISTER_POINT_STRUCT (LivoxPointXYZIT,
    (float, x, x) (float, y, y) (float, z, z) (float, intensity, intensity)
    (uint16_t, ring, ring) (float, time, time)
)
...
private:
    ...
    pcl::PointCloud<LivoxPointXYZIT>::Ptr tmpLivoxCloudIn; // ALM
...
public:
    ...
    void allocateMemory()
    {
        ...
        tmpLivoxCloudIn.reset(new pcl::PointCloud<LivoxPointXYZIT>()); // ALM
    ...
    bool cachePointCloud(const sensor_msgs::msg::PointCloud2::SharedPtr& laserCloudMsg)
    {
        ...
        {
            // Driver2 rosbag은 MID-360 line(0..3)을 adapter가 ring으로 보존한다.
            // live UDP처럼 ring이 없는 입력만 고도각을 N_SCAN 밴드로 양자화한다.
            ...
            bool hasNativeRing = false;
            for (const auto &field : currentCloudMsg.fields)
                hasNativeRing = hasNativeRing || field.name == "ring";
            constexpr float kVertMinDeg = -8.0f, kVertMaxDeg = 55.0f; // FOV 여유 포함
            for (size_t i = 0; i < tmpLivoxCloudIn->size(); i++)
            {
                ...
                {
                    float vertDeg = atan2(src.z, sqrt(src.x * src.x + src.y * src.y)) * 180.0f / M_PI;
                    int ring = int((vertDeg - kVertMinDeg) / (kVertMaxDeg - kVertMinDeg) * N_SCAN);
                    dst.ring = std::min(std::max(ring, 0), N_SCAN - 1);
                }
            }
        }
        ...
        if (ringFlag == 0)
        {
            ...
            if (ringFlag == -1)
            {
                if (sensor == SensorType::VELODYNE) {
                    ...
                    ringFlag = 1; // ALM: cachePointCloud에서 native ring 사용 또는 합성
                } else {
