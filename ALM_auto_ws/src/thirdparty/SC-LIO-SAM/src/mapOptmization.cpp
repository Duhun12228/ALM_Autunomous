...
// giseop (SC-LIO-SAM): Scan Context 루프클로저
...
public:
    ...
    pcl::PointCloud<PointType>::Ptr laserCloudRaw;   // giseop: deskewed raw (SC 입력)
    ...
    pcl::VoxelGrid<PointType> downSizeFilterSC; // giseop: SC 입력 다운샘플
    ...
    multimap<int, int> loopIndexContainer; // from new to old // giseop: RS+SC 중복 허용
    vector<pair<int, int>> loopIndexQueue;
    vector<gtsam::Pose3> loopPoseQueue;
    // giseop: SC 루프는 robust(Cauchy) 노이즈를 쓰므로 base 타입으로 다형성 유지
    vector<gtsam::SharedNoiseModel> loopNoiseQueue;
    deque<std_msgs::msg::Float64MultiArray> loopInfoVec;

    SCManager scManager; // giseop: Scan Context 루프 검출기
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("lio_sam_mapOptimization", options)
    {
        ...
        const float kSCFilterSize = 0.5; // giseop: SC 입력 다운샘플 leaf
    ...
    void laserCloudInfoHandler(const lio_sam::msg::CloudInfo::SharedPtr msgIn)
    {
        ...
        pcl::fromROSMsg(msgIn->cloud_deskewed, *laserCloudRaw); // giseop: SC 입력
        laserCloudRawTime = timeLaserInfoCur; // giseop
    ...
    void loopClosureThread()
    {
        ...
        {
            rate.sleep();
            performLoopClosure();      // RS(radius search) 루프
            performSCLoopClosure();    // giseop: Scan Context 루프
    ...
    void performLoopClosure()
    {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        // add loop constriant
        loopIndexContainer.insert(std::pair<int, int>(loopKeyCur, loopKeyPre)); // giseop: multimap
    }

    // giseop (SC-LIO-SAM): Scan Context 기반 루프 검출.
    // RS 루프(위)가 '위치가 가깝다'는 가정으로 찾는 반면, SC 는 드리프트로 위치가
    // 멀어져 있어도 스캔 형상 자체로 재방문을 인식한다. ICP 로 검증 후
    // robust(Cauchy) 노이즈 루프팩터를 추가한다.
    void performSCLoopClosure()
    {
        ...
        (void)detectResult.second; // yaw 초기값은 v1 미사용 (원본 SC-LIO-SAM 동일)
        ...
        pcl::PointCloud<PointType>::Ptr prevKeyframeCloud(new pcl::PointCloud<PointType>());
        {
            // 드리프트가 커도 ICP 가 붙도록 두 클라우드를 같은 기준(base_key)에서 조립
            int base_key = 0;
        ...
        // ICP Settings
        static pcl::IterativeClosestPoint<PointType, PointType> icp;
        icp.setMaxCorrespondenceDistance(150); // giseop: 2*historyKeyframeSearchNum 범위 커버 값
        icp.setMaximumIterations(100);
        icp.setTransformationEpsilon(1e-6);
        icp.setEuclideanFitnessEpsilon(1e-6);
        icp.setRANSACIterations(0);
        ...
        // giseop: 두 클라우드가 같은 기준으로 조립됐으므로 ICP 결과가 곧 상대 pose
        pcl::getTranslationAndEulerAngles(correctionLidarFrame, x, y, z, roll, pitch, yaw);
        gtsam::Pose3 poseFrom = Pose3(Rot3::RzRyRx(roll, pitch, yaw), Point3(x, y, z));
        gtsam::Pose3 poseTo = Pose3(Rot3::RzRyRx(0.0, 0.0, 0.0), Point3(0.0, 0.0, 0.0));

        // giseop: SC 루프는 오탐 가능성이 있어 robust 커널(Cauchy)로 완충
    ...
    // giseop (SC-LIO-SAM): 키프레임 주변 클라우드를 _wrt_key pose 기준으로 조립.
    // SC 루프는 드리프트로 두 지점의 절대 pose 가 어긋나 있으므로, 각자 자기
    // pose 로 조립하면 ICP 초기오차가 커진다. 같은 기준으로 조립하면 ICP 결과가
    // 곧 두 키프레임 간 상대 보정이 된다.
    ...
    void addLoopFactor()
    {
        ...
        for (int i = 0; i < (int)loopIndexQueue.size(); ++i)
        {
            ...
            auto noiseBetween = loopNoiseQueue[i]; // giseop: Diagonal(RS)/Robust(SC) 다형성
    ...
    void saveKeyFramesAndFactor()
    {
        ...
        // giseop (SC-LIO-SAM): 키프레임마다 SC 디스크립터 생성 (SINGLE_SCAN_FULL:
        // deskewed raw 다운샘플 사용 — 특징점보다 형상 정보가 풍부해 MID-360 에 유리)
