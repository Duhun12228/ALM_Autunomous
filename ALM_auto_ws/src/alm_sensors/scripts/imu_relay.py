...
livox_udp_imu.py 는 MID-360 내장 6축 IMU 를 sensor_msgs/Imu 로 /livox/imu 에
발행합니다. 이 IMU 는 자이로+가속도만 제공하고 방향(orientation) 추정값이 없으므로,
여기서 orientation_covariance[0] = -1 로 설정해 "orientation 없음"을 명시합니다.
...
class ImuRelay(Node):
    def __init__(self):
        ...
        # UDP IMU publisher 와 맞춰 best-effort QoS 로 구독한다.
